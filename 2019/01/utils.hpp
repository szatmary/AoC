...
public:
    ...
    // Creatse a grid of width w and height h, filled with character c
    grid(int w, int h, char c = ' ')
    {
        for (int y = 0; y < h; ++y) {
            _data.push_back(std::string(w, c));
        }
    }
    ...
    char get(point p) const noexcept { return is_in_bounds(p) ? _data[p.y][p.x] : '\0'; }
    void set(point p, char c = '*') noexcept
    {
        if (is_in_bounds(p)) {
            _data[p.y][p.x] = c;
        }
...
std::pair<std::smatch, bool> match(const std::string& regex, const std::string& input)
{
    std::smatch m;
    std::regex re(regex);
    auto ok = std::regex_match(input, m, re);
    return { m, ok };
}
